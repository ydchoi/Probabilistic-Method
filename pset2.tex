...
as $T_{(e,f)}$,
Note that the probability of $T_{(e,f)}$ is given by
\eQb
P(T_{(e,f)}) &=&  
...
with the given edge configuration,
the intersection between $e$ and $f$ does not have a single vertex, which gives $0$
...
\begin{lemma}[5.1. An equivalent condition for failure]
\textit{
The algorithm fails, if and only if there exists a conflicting pair $(e,f)$
...
Hence, failure occurs, if and only if there exists an edge, having all but one color
...
monochromatic coloring, if only if there exists a monochromatic edge containing the one vertex
...
\eQb
\{ \text{ failure probability} \} &=& \bigcup_{(e,f) \in E \times E} T_{(e,f)}. 
\eQe
...
&=& P(\bigcup_{(e,f) \in E\times E} T_{(e,f)}) \\ 
