...
\eQb
E[X - Y] &=& n^2 p - {n \choose 3}^2 p^{9} = m(p). 
\eQe
Hence, there exists a random assignment, for which the number of $1$s minus 
the number of $3 \times 3$ submatrices of $1$ is at least $m(p)$.
Fix such a coloring. Select one entry from each submatrix and change to $0$. This leaves
the matrix with at least $m(p)$ vertices. 

\smallskip

We now optimize this result by maximizing $m(p)$ with respect to $p$. Observe that $m$ 
is concave with respect to $p \in [0,1]$. Solving for the local maxima by setting the
first-order derivative equals to $0$, we get that $m$ is maximized at 
$p^* = (\dfrac{n^2}{9{n \choose 3}^2})^{\frac{1}{8}} = (\dfrac{2}{(n-1)(n-2)})^{\frac{1}{4}}$.
Substituting $p^*$ back into $m(p)$, we obtain
\eQb
m(p^*) &=& n^2(\dfrac{2}{(n-1)(n-2)})^{\frac{1}{4}} - {n \choose 3}^2 
