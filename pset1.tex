...
\eQb
(\bigvee_{i=1}^{n} \{ A_i \text{ is monochromatic }\})^c &=& 
\bigwedge_{i=1}^{n} \{ A_i \text{ is monochromatic }\}^c \\
...
\eQb
P( \bigvee_{i=1}^{n} \{ A_i \text{ has at most 3 colors }\} ) 
&\leq& \sum_{i=1}^{m} P(\{A_i \text{ has at most 3 colors }\}) \\
&=& m \cdot 3^{{n \choose 2}} 4^{ -{n \choose 2}}.
\eQe
which primarily grants us
\eQb
P( \bigvee_{i=1}^{n} \{ A_i \text{ is monochromatic }\} ) < 1. 
